...
export default function Cart() {
	...
	const handleRemove = indexPurchase => async () => {
		...
		await dispatch(getCartPurchases()).then(unwrapResult)
		toast.success('Đã xóa đơn hàng này rồi ạ', {
			position: 'top-center',
			autoClose: 3000,
	...
	const handleRemoveManyPurchases = async () => {
		...
		if (purchase_ids.length === checkedPurchases.length) {
			toast.success(`Đã xóa hết đơn hàng luôn rồi ạ`, {
				position: 'top-center',
				autoClose: 3000,
				hideProgressBar: false,
				closeOnClick: true,
			...
			toast.success(`Đã xóa các đơn hàng yêu cầu rồi ạ`, {
				position: 'top-center',
				autoClose: 3000,
				hideProgressBar: false,
				closeOnClick: true,
	...
	const handleBuyPurchases = async () => {
		if (checkedPurchases.length > 0) {
			...
			await dispatch(getCartPurchases()).then(unwrapResult)
			toast.success('Em chốt đơn rồi nha !', {
				position: 'top-center',
				autoClose: 3000,
				hideProgressBar: false,
				closeOnClick: true,
	...
	return (
		<div className="container">
			<Helmet>
				<title>
					ShopKing Việt Nam | mua và bán trên ứng dụng di dộng hoặc website
				</title>
			</Helmet>
			<div>
				<S.ProductHeader>
