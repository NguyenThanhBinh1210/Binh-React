...
export default function Header() {
	...
	const search = e => {
		e.preventDefault()
		history(path.home + `?name=${searchValue}`)
	}
	...
	return (
		<S.StyleHeader>
			<div className="container">
				<Navbar />
				<S.SearchWrapper>
					<S.Logo to="/">KingShop</S.Logo>
					<S.StyleForm onSubmit={search}>
						<S.StyleInput
							value={searchValue}
							onChange={onChangeSearch}
							placeholder="Tìm kiếm sản phẩm"
						/>
						<S.StyleButton type="submit">
							<svg
								height={19}
								viewBox="0 0 19 19"
								width={19}
					...
					<S.Cart onMouseEnter={showPopover} onMouseLeave={hidePopover}>
						<S.CartContainer>
							...
							<Popover active={activePopover}>
								<S.PopoverContent>
									<S.PopoverTitle>Sản phâm mới thêm</S.PopoverTitle>
									...
									<S.PopoverFooter>
										<S.MoreProduct>
											{purchases.length > 5 && (
												<span>{purchases.length - 5} Thêm hàng vào giỏ</span>
											)}
										</S.MoreProduct>
